...
use ncollide3d::{bounding_volume::BoundingVolume, partitioning::{BVH, SimultaneousVisitor, VisitStatus}, query::{PointQuery, RayCast}, shape::Shape};
use ncollide3d::bounding_volume::{AABB, self};
use ncollide3d::shape::{Segment, Plane};
use ncollide3d::query::{self, Ray};
use nalgebra::{Isometry3, Unit, Matrix3};

use crate::{Pt3, Vec1, Vec3};

use super::{Element, Plc, VertexId, Vertex, EdgeId, FaceId};

static mut DEBUG: bool = false;

fn point_in_face<V, E, F>(plc: &Plc<V, E, F>, point: Pt3, face: Vec<Vec<(VertexId, &Vertex<V>)>>, center: Pt3, normal: Unit<Vec3>) -> bool {
    ...
    // Check each polygon
    face.into_iter().map(|ring| {
        let mut ring = ring.into_iter().map(|(_, v)| transform * v.position()).collect::<Vec<_>>();
        ring.push(ring[0]);
        // Calculate intersection count
        ring.windows(2).map(|vertices| {
            ...
            let result = (pts[1].y <= pts[0].y && pts[0].y < pts[2].y &&
                sim::orient_2d(&pts, |l, i| l[i].coords, 0, 1, 2)) as usize;
            unsafe { if DEBUG { println!("Result: {}", result ); }}
            result
        }).sum::<usize>() % 2
    }).sum::<usize>() % 2 != 0
}
...
impl<'a, V, E, F> SimultaneousVisitor<Element, AABB<f64>> for IntersectionTest<'a, V, E, F> {
    fn visit(&mut self, left_bv: &AABB<f64>, left_data: Option<&Element>, right_bv: &AABB<f64>, right_data: Option<&Element>) -> VisitStatus {
        ...
        match (left, right) {
            (Element::Vertex(v0), Element::Vertex(v1)) => {
                if v0 < v1 {
                    let dist_squared = (self.plc[*v0].position() - self.plc[*v1].position()).norm_squared();
                    if dist_squared < self.tolerance * self.tolerance {
                        self.intersections.push((Element::Vertex(*v0), Element::Vertex(*v1), dist_squared.sqrt()));
                    }
                }
            }
            ...
            (Element::Vertex(v), Element::Edge(e)) => {
                ...
                if !ev.contains(v) {
                    ...
                    let dist = edge.distance_to_point(&Isometry3::identity(), &self.plc[*v].position(), true);
                    if dist < self.tolerance {
                        self.intersections.push((Element::Vertex(*v), Element::Edge(*e), dist));
                    }
                }
            }
            ...
            (Element::Edge(e0), Element::Edge(e1)) => {
                ...
                if ev0 < ev1 && !ev0.contains(&ev1[0]) && !ev0.contains(&ev1[1]) {
                    ...
                    let dist = query::distance(&Isometry3::identity(), &s0, &Isometry3::identity(), &s1);
                    if dist < self.tolerance {
                        self.intersections.push((Element::Edge(*e0), Element::Edge(*e1), dist));
                    }
                }
            }

            (Element::Vertex(v), Element::Face(f)) => {
                let fv = self.plc[*f].vertices(self.plc).map(Vec::from_iter).collect::<Vec<_>>();
                if !fv.iter().flatten().any(|(vertex, _)| v == vertex) {
                    ...
                    let dist = plane.distance_to_point(&Isometry3::translation(center.x, center.y, center.z), &p0, false).abs();
                    if dist < self.tolerance && point_in_face(self.plc, p0, fv, center, plane.normal) {
                        self.intersections.push((Element::Vertex(*v), Element::Face(*f), dist));
                    }
                    unsafe { if DEBUG { DEBUG = false; }}
                }
            }
            ...
            (Element::Edge(e), Element::Face(f)) => {
                let fv = self.plc[*f].vertices(self.plc).map(Vec::from_iter).collect::<Vec<_>>();
                let ev = self.plc[*e].vertices();
                if !fv.iter().flatten().any(|(vertex, _)| ev[0] == *vertex || ev[1] == *vertex) {
                    let plane = Plane::new(self.plc[*f].plane.1);
                    ...
                    // Why does Isometry3::translation take 3 separate arguments?!
                    if let Some(toi) = 
                        plane.toi_with_ray(&Isometry3::translation(center.x, center.y, center.z), &ray, 1.0, false)
                    {
                        let point = positions[0] + (positions[1] - positions[0]) * toi;
                        if point_in_face(self.plc, point, fv, center, normal) {
                            self.intersections.push((Element::Edge(*e), Element::Face(*f), 0.0));
                        }
                    }
                }
            }
            ...
            (Element::Face(_), Element::Edge(_)) | (Element::Face(_), Element::Vertex(_)) | (Element::Edge(_), Element::Vertex(_)) => {}
...
pub(super) fn vertex_bound<V, E, F>(plc: &Plc<V, E, F>, vertex: VertexId, tolerance: f64) -> (Element, AABB<f64>) {
...
pub(super) fn edge_bound<V, E, F>(plc: &Plc<V, E, F>, edge: EdgeId, tolerance: f64) -> (Element, AABB<f64>) {
...
pub(super) fn face_bound<V, E, F>(plc: &Plc<V, E, F>, face: FaceId, tolerance: f64) -> (Element, AABB<f64>) {
